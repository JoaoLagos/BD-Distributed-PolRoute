def execute_query(spark, save_output=False, output_format="csv"):
    ...
    """
    print("\nExecutando Query 2 (corrigida)...")
    result = spark.sql(query)

    if save_output:
        result.write.format(output_format).option("header", True).mode("overwrite").save("results/query_02")
        print("Resultados salvos em results/query_02")
